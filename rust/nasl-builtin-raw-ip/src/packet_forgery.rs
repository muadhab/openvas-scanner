...
fn print_tcp_packet(
    tcp: &Option<packet::tcp::TcpPacket>,
    configs: &Context,
) -> Result<(), FunctionErrorKind> {
    match tcp {
        Some(pkt) => {
            ...
            f.push_str("\tth_flags : ");
            if flags & pnet::packet::tcp::TcpFlags::FIN == pnet::packet::tcp::TcpFlags::FIN {
                f.push_str("TH_FIN");
            }
            if flags & pnet::packet::tcp::TcpFlags::SYN == pnet::packet::tcp::TcpFlags::SYN {
                if !f.is_empty() {
                    f.push('|')
                };
                f.push_str("TH_SYN");
            }
            if flags & pnet::packet::tcp::TcpFlags::RST == pnet::packet::tcp::TcpFlags::RST {
                if !f.is_empty() {
                    f.push('|')
                };
                f.push_str("TH_RST");
            }
            if flags & pnet::packet::tcp::TcpFlags::PSH == pnet::packet::tcp::TcpFlags::PSH {
                if !f.is_empty() {
                    f.push('|')
                };
                f.push_str("TH_PSH");
            }
            if flags & pnet::packet::tcp::TcpFlags::ACK == pnet::packet::tcp::TcpFlags::ACK {
                if !f.is_empty() {
                    f.push('|')
                };
                f.push_str("TH_ACK");
            }
            if flags & pnet::packet::tcp::TcpFlags::URG == pnet::packet::tcp::TcpFlags::URG {
                if !f.is_empty() {
                    f.push('|')
                };
                f.push_str("TH_URG");
        ...
        None => Err(FunctionErrorKind::WrongArgument(
...
fn dump_udp(
    datagram: &Option<packet::udp::UdpPacket>,
    data: &[u8],
    configs: &Context,
) -> Result<NaslValue, FunctionErrorKind> {
    match datagram {
    ...
    for udp_datagram in positional.iter() {
        match udp_datagram {
            NaslValue::Data(data) => {
                ...
                dump_udp(&datagram, data, configs)?;
            }
            _ => {
                return Err(FunctionErrorKind::WrongArgument(
                    "Invalid UDP packet".to_string(),
...
fn forge_ip_v6_packet(
    ...
    let mut buf = vec![0; total_length];
    let mut pkt = packet::ipv6::MutableIpv6Packet::new(&mut buf).ok_or_else(|| {
        FunctionErrorKind::Diagnostic(
            "No possible to create a packet from buffer".to_string(),
            None,
        )
...
fn get_ip_v6_element(
    ...
    let buf = match register.named("ip6") {
        Some(ContextType::Value(NaslValue::Data(d))) => d.clone(),
        _ => {
            return Err(FunctionErrorKind::from("ip6"));
        }
    };
...
fn insert_ip_v6_options(
    ...
    let mut new_pkt = MutableIpv6Packet::new(&mut new_buf).ok_or_else(|| {
        FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
    })?;
    ...
    for ip in positional.iter() {
        match ip {
            NaslValue::Data(data) => {
                ...
                configs
                    .logger()
                    .info(&format!("\tip6_v  : {:?}", pkt.get_version()));
                configs
                    .logger()
                    .info(&format!("\tip6_tc   : {:?}", pkt.get_traffic_class()));
                configs
                    .logger()
                    .info(&format!("\tip6_fl : {:?}", pkt.get_flow_label()));
                configs
                    .logger()
                    .info(&format!("\tip6_plen : {:?}", pkt.get_payload_length()));
                configs
                    .logger()
                    .info(&format!("\tip6_hlim  : {:?}", pkt.get_hop_limit()));
                ...
                match pkt.get_next_header() {
                    ...
                    IpNextHeaderProtocols::Icmpv6 => configs.logger().info(&format!(
                ...
                configs
                    .logger()
                    .info(&format!("\tip6_src : {:?}", pkt.get_source().to_string()));
                configs.logger().info(&format!(
                    "\tip6_dst : {:?}",
                    pkt.get_destination().to_string()
                ));
...
fn forge_tcp_v6_packet(
    ...
    let chksum = match register.named("th_sum") {
        Some(ContextType::Value(NaslValue::Number(x))) if *x != 0 => (*x as u16).to_be(),
        _ => {
            let pkt = packet::ipv6::Ipv6Packet::new(&ip_buf).ok_or_else(|| {
                FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
            })?;
            let tcp_aux = TcpPacket::new(tcp_seg.packet()).ok_or_else(|| {
                FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
    ...
    ip_buf.append(&mut buf);
    let l = ip_buf.len();
    let mut pkt = packet::ipv6::MutableIpv6Packet::new(&mut ip_buf).ok_or_else(|| {
        FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
    })?;
...
fn get_tcp_v6_element(
    ...
    let ip = packet::ipv6::Ipv6Packet::new(&buf).ok_or_else(|| {
        FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
    })?;
...
fn set_tcp_v6_elements(
    ...
    let l = new_ip_buf.len();
    let mut pkt = packet::ipv6::MutableIpv6Packet::new(&mut new_ip_buf).ok_or_else(|| {
        FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
    })?;
...
fn forge_udp_v6_packet(
    ...
    let mut ip_buf = match register.named("ip6") {
        Some(ContextType::Value(NaslValue::Data(d))) => d.clone(),
        _ => return Err(FunctionErrorKind::from("ip6")),
    };
    let original_ip_len = ip_buf.len();
...
fn forge_icmp_v6_packet(
    ...
    let mut ip_buf = match register.named("ip6") {
        Some(ContextType::Value(NaslValue::Data(d))) => d.clone(),
        _ => {
            return Err(FunctionErrorKind::from("icmp6"));
        }
    };

    let original_ip_len = ip_buf.len();
    // to extract the max hop limit, ip6_dst.
    let pkt_aux = packet::ipv6::Ipv6Packet::new(&ip_buf).ok_or_else(|| {
        FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
    })?;
    ...
    let icmp_type = match register.named("icmp_type") {
        Some(ContextType::Value(NaslValue::Number(x))) => {
            if *x < 0 || *x > 255 {
                return Err(FunctionErrorKind::Dirty(format!(
                    "forge_icmp_v6_packet: illegal type {}",
                    x
                )));
            }
        ...
        _ => {
            return Err(FunctionErrorKind::Dirty(
                "forge_icmp_v6_packet: illegal type".to_string(),
            ));
        }
    };
    ...
    let chksum = match register.named("icmp_cksum") {
        Some(ContextType::Value(NaslValue::Number(x))) if *x != 0 => (*x as u16).to_be(),
        _ => {
            let icmp_aux = Icmpv6Packet::new(&icmp_buf).ok_or_else(|| {
                FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
            })?;
...
fn get_icmp_v6_element(
    ...
    let icmp = packet::icmpv6::Icmpv6Packet::new(ip.payload()).ok_or_else(|| {
        FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
    })?;
...
fn dump_icmp_v6_packet(
    ...
    for icmp_pkt in positional.iter() {
        ...
        configs
            .logger()
            .info(&format!("\ticmp6_id    : {:?}", icmp_id));
        configs
            .logger()
            .info(&format!("\ticmp6_code  : {:?}", icmp.get_icmp_code()));
        configs
            .logger()
            .info(&format!("\ticmp_type  : {:?}", icmp.get_icmp_type()));
        configs
            .logger()
            .info(&format!("\ticmp6_seq   : {:?}", icmp_seq));
        configs
            .logger()
            .info(&format!("\ticmp6_cksum : {:?}", icmp.get_checksum()));
        configs.logger().info(&format!("\tData       : {:?}", data));
        configs.logger().info(&"\n");
    }
    Ok(NaslValue::Null)
...
fn nasl_tcp_v6_ping(
    ...
    let mut ip_buf = [0u8; 40];
    let mut ip = MutableIpv6Packet::new(&mut ip_buf).ok_or_else(|| {
        FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
    })?;
...
fn nasl_send_v6packet(
    ...
    for pkt in positional.iter() {
        ...
        let packet = packet::ipv6::Ipv6Packet::new(packet_raw).ok_or_else(|| {
            FunctionErrorKind::Dirty("No possible to create a packet from buffer".to_string())
        })?;
...
pub fn lookup(key: &str) -> Option<NaslFunction> {
    match key {
        ...
        //IPv6
        "forge_ip_v6_packet" => Some(forge_ip_v6_packet),
        "get_ip_v6_element" => Some(get_ip_v6_element),
        "set_ip_v6_elements" => Some(set_ip_v6_elements),
        "insert_ip_v6_options" => Some(insert_ip_v6_options),
        "dump_ip_v6_packet" => Some(dump_ip_v6_packet),
        "forge_tcp_v6_packet" => Some(forge_tcp_v6_packet),
        "get_tcp_v6_element" => Some(get_tcp_v6_element),
        "get_tcp_v6_option" => Some(get_tcp_v6_option),
        "set_tcp_v6_elements" => Some(set_tcp_v6_elements),
        "insert_tcp_v6_options" => Some(insert_tcp_v6_options),
        "dump_tcp_v6_packet" => Some(dump_tcp_v6_packet),
        "tcp_v6_ping" => Some(nasl_tcp_v6_ping),
        "forge_udp_v6_packet" => Some(forge_udp_v6_packet),
        "get_udp_v6_element" => Some(get_udp_v6_element),
        "set_udp_v6_elements" => Some(set_udp_v6_elements),
        "dump_udp_v6_packet" => Some(dump_udp_v6_packet),
        "forge_icmp_v6_packet" => Some(forge_icmp_v6_packet),
        "get_icmp_v6_element" => Some(get_icmp_v6_element),
        "dump_icmp_v6_packet" => Some(dump_icmp_v6_packet),
        "forge_igmp_v6_packet" => Some(forge_igmp_v6_packet),
        "send_v6packet" => Some(nasl_send_v6packet),
        _ => None,
    }
}
